...
// Получает с БД массив со всеми пользователями и отправляет его в вид user_tests.php
    public function actionUser_names() {
	$list = User::find()->asArray()->select(['username', 'id', 'email'])->where(['role' => 'user'])->all();
	$listuser = [];
	foreach ($list as $key => $value) {
	    $listuser[$value['email']] = $value['username'];
	}
	return $this->render('user_names', compact('listuser'));
    }

    // Вывод (просмотр) выбранного из списка созданных тестов 
	...
	// }
	return $this->render('view_tests_name', compact('arr_words', 'comment', 'name_test'));
